...
Typicially IP spoofing has limited use during real attacks outside DoS. Any TCP
...
from one or more IP addresses. When specifiying IP addresses you can choose to
sequently or randomly go through a range of source addresses.
...
source IP addresses by altering the inital source address.
...
The bits set do not have to be sequental like a subnet mask. For example the
mask 0.128.1.255 is valid and will explore the ranges 10.0,128.0-1.0-255.

When checking a range of ipaddresses randomly or sequentally it requires you to
enter the path to the root of the tftp directory. The script will check this
directory to see if the file has been sucessful transfered.
...
If the tester doesnt know the details of they could try and guess.
...
possible to determine which source address was successful, but dermines one
...
which should be used. The mode names are the same execpt have a '_l' suffix.

For example to repeat the same attack using a list of community strings in in
list.txt the following arguements should be used.
...
As programming langauges go Python is a simple langauge, easy to read and write
and I encourage you to attempt to debug and correct any issues you find and
send me your changes so I can share them with other users on the NCC Git hub.
